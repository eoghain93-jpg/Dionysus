// Server-side monthly report builder.
//
// Deno port of src/lib/monthlyReport.js so the scheduled send doesn't need
// a till open in a browser. Keep the two in sync: same cash-basis rules,
// same CSV shape, same column order — the accountant's spreadsheet imports
// depend on it.
//
...
// Tab orders are IOUs, excluded from revenue; what's owed surfaces as
// Outstanding Tabs. Wastage and staff drinks are valued at standard_price.
...
/** The month that just ended, in YYYY-MM — the scheduled send's default. */
export function previousMonthISO(now = new Date()): string {
...
// PostgREST caps every response at 1000 rows. A month of pub trading is
...
export async function fetchMonthlyReportData(
  ...
  const retailValue = (rows: Row[]) =>
    sum(rows, r => Number(r.quantity ?? 0) * Number(r.products?.standard_price ?? 0))
...
export function toCsv(data: MonthlyReportData): string {
  const lines: string[] = []
