-- Base schema as a replayable migration.
--
-- supabase/schema.sql predates the migration ledger and was applied to
-- production by hand, so the migrations that follow cannot assume these
-- tables exist on a fresh database (20260319000000_member_auth.sql already
-- references members/orders). This migration replays schema.sql, but only on
-- databases where the base tables are missing. On production — where later
-- migrations have since dropped the "Allow all" policies and revoked
-- adjust_stock — it records as applied and changes nothing, so it must NOT
-- recreate any of those objects when members already exists.
...
begin
  if to_regclass('public.members') is not null then
    raise notice 'base schema already present; skipping';
    return;
  end if;
  ...
  -- Open policies, matching the original schema.sql state. Later migrations
  -- (20260319000000_member_auth.sql, 20260610170000_till_device_rls.sql)
  -- replace these, exactly as happened on production.
